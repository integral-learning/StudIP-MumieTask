...
class TaskController extends StudipController
{
    ...
    public function index_action()
    {
        if (!$this->task->is_graded) {
            PageLayout::postInfo(dgettext("MumieTaskPlugin", "Für diese Aufgabe werden keine Noten in Stud.IP erfasst"));
            return;
        }
    ...
    {
        if (!$this->task->is_graded) {
            PageLayout::postError(dgettext("MumieTaskPlugin", "Für diese Aufgabe werden keine Noten erfasst. Bitte kehren Sie zur Übersichtsseite zurück!"));
            return;
        }
        $this->addGradeInfoSidebar();
        if ($this->hasTeacherPermission) {
